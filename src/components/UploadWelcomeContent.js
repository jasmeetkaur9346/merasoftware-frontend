...
const UploadWelcomeContent = ({ onClose, fetchData }) => {
    ...
    const handleAddGuestSlide = () => {
        if (data.guestSlides.length < 2) { // Maximum 2 slides
            setData(prev => ({
                ...prev,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(SummaryApi.uploadWelcomeContent.url, {
                method: SummaryApi.uploadWelcomeContent.method,
                credentials: 'include',
                headers: {
            ...
            toast.error('Error uploading welcome content');
        }
    };
        ...
        <div className='fixed w-full h-full bg-slate-200 bg-opacity-40 top-0 left-0 right-0 bottom-0 flex justify-center items-center'>
            <div className='bg-white p-4 rounded w-full max-w-2xl h-full max-h-[75%] overflow-hidden'>
                <div className='flex justify-between items-center pb-3'>
                    <h2 className='font-bold text-lg'>Upload Welcome Content</h2>
...
export default UploadWelcomeContent
